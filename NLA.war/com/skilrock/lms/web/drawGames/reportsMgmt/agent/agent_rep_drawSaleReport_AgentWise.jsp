...
<div align="center">
			<h3>
				<u><s:text name="label.draw.sale"/> <s:text name="Report"/> <s:property value="%{reportType}"/>&nbsp; <s:text name="label.from"/> : <s:property value="startDate" />
...
<s:if test="#session.reportList.size>0">
				<s:div id="excel"
					...
					<s:a href="agent_rep_drawSaleReport_exportExcel.action?reportType=Retailer Wise&start_date=%{#startDate}&end_Date=%{#endDate}&agentOrgId=%{#agtOrgId}"><s:text name="btn.exportasexcel"/></s:a>
				</s:div>
</s:if>
	...
	<s:if
		...
		<s:iterator id="result" value="#session.reportList" status="index">
			<tr class="startSortable">
				...
				<td nowrap="nowrap">
					<s:a
						href="agent_rep_drawSaleReport_SearchExpand.action?reportType=Retailer Wise Expand&start_date=%{#startDate}&end_Date=%{#endDate}&agentOrgId=%{gameNo}"
...
$RCSfile: agent_rep_drawSaleReport_AgentWise.jsp,v $
