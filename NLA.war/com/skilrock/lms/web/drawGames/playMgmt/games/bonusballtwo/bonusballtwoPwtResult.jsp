...
<%
	pageContext.setAttribute("gameName", "BonusBallTwo");
	...
	if (isReprint) {
		...
		if (bean != null) {
			...
			if("Perm5".equalsIgnoreCase(bean.getPlayType())){
			...
			if("YES".equalsIgnoreCase((String)application.getAttribute("PERM_COMB_DISP"))){
				...
				if("Perm5".equalsIgnoreCase(bean.getPlayType())){
					comboList = Util.rec(0, 5, 0, new String[5], pickNumStr.toString().replace(";","").split(","), new StringBuffer(""), comboList);
					compDispTemp = comboList.toString().replace("[", "").replace("]", "").replace(" ", "").replace(",Nxt,", "Nxt").replace(",Nxt", "");
					appletHeight = appletHeight + (6*comboList.size()/2);
				}
			...
			reprintData = "data=" + bean.getTicket_no() + bean.getReprintCount()
							...
							+ "|unitPrice=" + bean.getUnitPrice()*bean.getBetAmtMultiple()
							...
							+ "|jackPotAmt=" + ((Map)application.getAttribute("jackPotMap")).get(bean.getGame_no())
...
document.write "<object ID='WB' WIDTH=0 HEIGHT=0 CLASSID='CLSID:8856F961-340A-11D0-A96B-00C04FD705A2'></object>"
	...
	<body>
		...
		<div id="maincontainer">
			...
			<div class="drawgame_name">BONUSBALL TWO</div>
		...
		System.out.println("BONUSBALLTWO Winning DATA: " + finalData.toString());
	}
	%>		
...
$RCSfile: bonusballtwoPwtResult.jsp,v $
$Revision: 1.1.2.5 $
</code>