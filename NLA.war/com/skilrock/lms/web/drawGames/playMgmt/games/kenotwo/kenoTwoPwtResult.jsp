		...
		appletHeight = UtilApplet.getAdvMsgs(kenoBean.getAdvMsg(), topMsgsStr, bottomMsgsStr, appletHeight);
				...
				if(playerDataApp.length > 0){
					...
					for (int panelId = 0; panelId < playerDataApp.length; panelId++) {
						if(playType[panelId].contains("Banker")){
							playerDataApp[panelId] = playerDataApp[panelId].replace("-1","XX");
						}
						...
						if("Perm2".equalsIgnoreCase(playType[panelId])){
							...
							compDispTemp = comboList.toString().replace("[", "").replace("]", "").replace(" ", "").replace(",Nxt,", "Nxt").replace(",Nxt", "");
							appletHeight = appletHeight + (5*comboList.size()/2);
						}
			...
			System.out.print(Util.getGameMap().get(kenoBean.getGameId()).getTicketExpiryPeriod()+"TKT EXP");
...
document.write "<object ID='WB' WIDTH=0 HEIGHT=0 CLASSID='CLSID:8856F961-340A-11D0-A96B-00C04FD705A2'></object>"
	...
	<body>
		...
		<%
			...
			System.out.println("KENO TWO Winning DATA: " + finalData.toString());
		 %>
...
$RCSfile: kenoTwoPwtResult.jsp,v $
$Revision: 1.1.2.14.4.3.2.1 $
</code>