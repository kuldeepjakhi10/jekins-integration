...
<%
	pageContext.setAttribute("gameName", "Zimlottotwo");
	...
	if (isReprint) {
		...
		if (bean != null) {
			...
			String totalQuantity = bean.getPanel_id().length +"";
			if("Perm6".equalsIgnoreCase(bean.getPlayType())){
				totalQuantity=bean.getNoOfLines()+"";
			}
			
			reprintData = "data=" + bean.getTicket_no() + bean.getReprintCount() + (bean.getBarcodeCount()!=-1 && LMSFilterDispatcher.isBarCodeRequired?bean.getBarcodeCount():"")
							...
							+ "|expiryPeriod=" + Util.getGameMap().get(bean.getGameId()).getTicketExpiryPeriod()
							+ "|noOfDraws=" + bean.getNoOfDraws()
							+ "|playType=" + bean.getPlayType()
							...
							+ "|jackPotAmt=" + ((Map)application.getAttribute("jackPotMap")).get(Util.getGameId((String)pageContext.getAttribute("gameName")))
...
<%
	...
	for (int i = 0; i < winningBean.getDrawWinList().size(); i++) {
		...
		clmPendAmt[i]=0.0;
			...
			clmPendAmtStr.append(clmPendAmt[i] + ",");	
			winStr.append(win[i] + ",");
			clmStr.append(clm[i] + ",");
...
document.write "<object ID='WB' WIDTH=0 HEIGHT=0 CLASSID='CLSID:8856F961-340A-11D0-A96B-00C04FD705A2'></object>"
	...
	<body>
		...
		<div id="maincontainer">
			...
			<div class="drawgame_name">ZIMLOTTO Bonus</div>
		...
		System.out.println("ZIMLOTTO BONUS Winning DATA: " + finalData.toString());
	}
	%>		
...
$RCSfile: zimlottobonustwoPwtResult.jsp,v $
