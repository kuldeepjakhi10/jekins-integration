...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr>
		<td>
			<%
				Map<String, TreeMap<String, TreeMap<String, List<userPrivBean>>>> rolePrivMap = (TreeMap<String, TreeMap<String, TreeMap<String, List<userPrivBean>>>>) request
							.getAttribute("rolePriviledgeMap");
					...
					<td>
						<table id="inter_tbl<%=i%>"
							...
							<tr id="inter_row<%=i%>">
								...
								<th id="inter<%=i%>_col<%=j%>"
									onclick="changeTab('inter<%=i%>_col<%=j%>','priv<%=i%>_row<%=j%>',<%=privMap.size()%>)"
									onmouseover="this.style.cursor='hand';" height="30"
							...
							<tr id="priv<%=i%>_row<%=k%>" style="display: none">
								<td colspan="<%=j%>">
									<table id="tbl_priv<%=i%>_row<%=k%>" cellpadding="0"
										cellspacing="0" border="0" width="795">
										<tr>
											<td>
												<s:iterator value="#session.privMap" status="rowIndex">
													<table cellpadding="4" cellspacing="0" border="0"
														width="100%" bgcolor="#c3d8f2">
														<tr>
	...
	<tr>
		<td>
			<table>
				<tr>
					<td>
						<input type="button" value="<s:text name="btn.reset.priv"/>" class="button"
					...
					<td>
						<table>
							<s:submit key="btn.assign.priv" align="left"
								cssClass="button" />
						</table>
					</td>
				</tr>
...
<code id="headId" style="visibility: hidden">
	$RCSfile: bo_rm_editRole_FetchPriv.jsp,v $ $Revision: 1.11.8.4.10.3 $
</code>