...
<%
		...
		<td>
			<%
				...
				while (itrMap.hasNext()) {
					...
					i++;
			%>
	<table style="background: #eeeeee" border="0" cellpadding="0" cellspacing="0" width="830px">
		<tr id="ser_row<%=i%>">
	<% int colorcode;
						if(i%2 ==0)
						   colorcode =1;
	...
	<th height="25" id="ser_col<%=i%>" class="color<%=colorcode %>"	onclick="showHideTab('inter_tbl<%=i%>')" onmouseover="this.style.cursor='hand';">
	...
	<table id="inter_tbl<%=i%>" style="display: none;background: #eeeeee" border="0" cellpadding="0" cellspacing="0" width="100%" class="bdrLRB">
	 ...
	 <th id="inter<%=i%>_col<%=j%>" height="25" align="center" width="50%"	onclick="changeTab('inter<%=i%>_col<%=j%>','priv<%=i%>_row<%=j%>',<%=privMap.size()%>)"
		...
		<table cellpadding="4" cellspacing="0" border="0" width="100%">														<tr>
															<th style="background: #cadcf1; color: #333333">
																...
																<s:elseif test="%{key=='DRAW_MGT'}">Draw Management</s:elseif>
															</th>
														</tr>
														<tr
...
$RCSfile: bo_um_editBoMasPriv_FetchPriv.jsp,v $
$Revision: 1.1.2.5 $
</code>