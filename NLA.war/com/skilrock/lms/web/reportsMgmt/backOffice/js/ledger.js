...
function retrieveURL(url, div) {
   ...
   _resp  = _ajaxCall(url);
	if(_resp.result){
			...
			if (document.ledger.AgtLedger.value == "yes") {
				if (document.ledger.Agt.value == "1") {
					retrieveURL("bo_rep_BOLedger_FetchAccList.action?ledgerType=Agentwise", "list1");
				}
			}
      } else {
        alert("Problem: ");
...
function validate() {  	
	...
	//alert("In For");
		...
		if (e.name == "agentName") {//alert("In agentName");
			if (e.value == "-1") {
				alert('Please Select '+tierMap['AGENT']);
...
function onload() {
	...
	if (document.ledger.type.value == "agent" && isMulLedgerType=='false') {
		ledgerType = "Retailerwise";
		_id.non('selType');
		for (var i = 0; i < document.forms[0].elements.length; i++) {
			var e = document.forms[0].elements[i];
	...
	if (document.ledger.type.value == "agent") {
		retrieveURL("bo_rep_agentLedger_FetchAccList.action?ledgerType=" + ledgerType, "list");
	} else {
		retrieveURL("bo_rep_BOLedger_FetchAccList.action?ledgerType=Accountwise", "list");
	}
}
...
parent.frames[0].Version['$Source: /rep/LMS_Mgmt/WebRoot/com/skilrock/lms/web/reportsMgmt/backOffice/js/ledger.js,v $'] = '$Id: ledger.js,v 1.1.8.4 2011/03/22 11:39:00 sachin Exp $';