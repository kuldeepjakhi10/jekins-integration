...
<s:if test="#session.saletype!=null">
	 ...
	 <table width="100%" border="1" cellpadding="3" cellspacing="0" bordercolor="#CCCCCC" align="center">
		...
		<tr>	
			<th>
				<%if("GM".equalsIgnoreCase((String)session.getAttribute("saletype"))){ %>
					Game Name
				<%}else{ %> Party Name<%} %> 
			</th>			
			<th>Sale Amount</th>
	...
	<s:if test="#session.startValueOrderSearch!=0"><s:a theme="ajax" targets="down" href="bo_dg_rep_sale_Navigate.action?end=first">First</s:a><s:a theme="ajax" targets="down" href="bo_dg_rep_sale_Navigate.action?end=Previous" > Previous</s:a></s:if><s:else>First Previous</s:else>    
    <s:if test="#session.startValueOrderSearch==((#session.APP_ORDER_LIST5.size/10)*10)">Next Last</s:if><s:else><s:a theme="ajax" targets="down" href="bo_dg_rep_sale_Navigate.action?end=Next" >Next</s:a><s:a theme="ajax" targets="down" href="bo_dg_rep_sale_Navigate.action?end=last">Last</s:a></s:else>      
...
$RCSfile: bo_rep_DG_sale_SearchResult.jsp,v $
$Revision: 1.1.8.4 $
</code>