...
<body onload="disable()">
	...
	<s:if test="#session.searchCashChqResult.size>0">
			<s:div id="excel" cssStyle="text-align: left;width: 100%;font-size: 9pt"><s:a href="agt_rep_cashChq_ExpExcel.action">Export As Excel</s:a></s:div>
	</s:if>
	...
	<s:if test="#session.startValueOrderSearch!=0"><s:a theme="ajax" targets="down" href="agt_rep_cashChq_Navigate.action?end=first">First</s:a><s:a theme="ajax" targets="down" href="agt_rep_cashChq_Navigate.action?end=Previous" > Previous</s:a></s:if><s:else>First Previous</s:else>    
    <s:if test="#session.startValueOrderSearch==((#session.APP_ORDER_LIST5.size/10)*10)">Next Last</s:if><s:else><s:a theme="ajax" targets="down" href="agt_rep_cashChq_Navigate.action?end=Next" >Next</s:a><s:a theme="ajax" targets="down" href="agt_rep_cashChq_Navigate.action?end=last">Last</s:a></s:else>      
...
$RCSfile: agt_rep_cashChq_SearchResult.jsp,v $
$Revision: 1.1.8.5.8.1.2.1 $
</code>