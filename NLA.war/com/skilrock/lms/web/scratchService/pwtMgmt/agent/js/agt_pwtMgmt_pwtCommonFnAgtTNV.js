...
{     //   alert("inside check binding for game: " + c + "error div for tis game is "+ _id.o(c+'errorDiv'));
       var tbl = _id.o('game'+'Virn'+c);
       var elmnt=tbl.getElementsByTagName("input");
       var elmntLength =  (elmnt.length)/2+1;
      // alert(elmntLength);	   
		var isCrtBinding="yes";
		for(var d=1;d<elmntLength;d++)
			{
				  var ticketValue = _id.o(c+"ticketNbr"+d).value;
				  var virnValue =   _id.o(c+"virnCode"+d).value;
				  //alert('ticket: '+ticketValue + " :: virn " + virnValue);
				  if( (ticketValue!="" && virnValue!="")  || (ticketValue=="" && virnValue=="") )
				  {
				    //alert('ok');		
				    _id.hid(c+'errorDiv'+d);
			        _id.o(c+'errorDiv'+d).parentNode.style.backgroundColor=tbl.style.backgroundColor;
				    	     
				  }
				  else
				  {
				   // _id.o(c+'errorDiv').innerHTML = '<font color="red"><br/> binding is not perfect</font>';
				    _id.vis(c+'errorDiv'+d);
			        _id.o(c+'errorDiv'+d).parentNode.style.backgroundColor="#cccccc";isDp=true;
				    // alert('Binding is not perfect');count+'errorDiv'+counter
				    returnType = true;
				    isCrtBinding="no";
				  }
...
if(elmnt[i].value!=""&&!_mArr[i]){		   
		     ...
		     for(j=i+1; j<l; j++){ 
		     		if(elmnt[j].value!=""&&!_mArr[j]){
			           	...
			           	_id.vis(c+'errorDiv'+(j));
			           	_id.o(c+'errorDiv'+(j)).parentNode.style.backgroundColor="#cccccc";
			           	 isDp=true;
			           	_mArr[j]=true;
			          }else{
			          	_id.hid(c+'errorDiv'+(j));
			           	_id.o(c+'errorDiv'+(j)).parentNode.style.backgroundColor=tbl.style.backgroundColor;
			          }
			          }
		         	}
		   	 }		   	 
...
function checkDuplicateNew(type){
   //alert('inside new check fill');
	var isFill = true;
	var isDp = false;
	for(var c=1;c<count+1;c++){	   
	...
	//alert("is fill " + isFill);
	if(!isFill){
	  isDp = checkDup();
	  //alert('after check dup' + isDp);
	}
	//alert('58585858');
	//alert('is duplicate ' + isDP);		
	return {fill:isFill,dup:isDp}; 
}


function checkDup(){
   // alert('inside new check dup');	
	var isDp = false;
	for(var c=1;c<count+1;c++){	   
	    var tbl = _id.o('game'+'Virn'+c);
		var elmnt=tbl.getElementsByTagName("input");
	    ...
	    for(var i=0,l=elmnt.length;i<l;i++){		 
		   if(elmnt[i].value!=""){		   
		   // alert('input field number: '+i);
		     for(j=i+1; j<l; j++){
		       //  alert("input field match with number: " + j); 
		     		if(elmnt[j].value!=""){		     		 
		     		  var errrInputId=elmnt[j].id;		     		   
		     		  
		             // var length = errrInputId.length;		              		         
		   			 //  var erorCounter=	  errrInputId.substring(length-1,length);
		   			   
		   			    var erorCounter;
		   			  // alert("is Virn: " + errrInputId.match('virnCode'));
		     		   if(errrInputId.match('virnCode'))
		     		   {
		     		    //alert(errrInputId.split('virnCode')[1]);
		     		    erorCounter=errrInputId.split('virnCode')[1];
		     		   }
		     		   else{
		     		     // alert(errrInputId.split('ticketNbr')[1]);
		     		      erorCounter=errrInputId.split('ticketNbr')[1]
		     		   }
		     		   	
		   			//   alert('error counter' + erorCounter);	   			   
			           	if ((elmnt[i].value).replace(/-/g,'') == (elmnt[j].value).replace(/-/g,'')){			          
			           	_id.vis(c+'errorDiv'+erorCounter);			           
			           	_id.o(c+'errorDiv'+erorCounter).parentNode.style.backgroundColor="#cccccc";			          
			           	 isDp=true;				                   	
			          }/*else{			          
			          	_id.hid(c+'errorDiv'+erorCounter);
			           	_id.o(c+'errorDiv'+erorCounter).parentNode.style.backgroundColor=tbl.style.backgroundColor;
			          }*/
			          }
		         	}
		   	 }		   	 
	...
	//alert("inside checkDup is dup is " + isDp);		
	return isDp;
...
function checkDuplicate(type){
  ...
  // alert("yogesh "+counter);
	var isFill = true;var isDp = false;
	for(var c=1;c<count+1;c++){
	   //alert("game: " + c);
	    var tbl = _id.o('game'+type+c);
		var elmnt=tbl.getElementsByTagName("input");var _mArr = new Array();
		//alert("total element: " + elmnt.length);
	    for(var i=0,l=elmnt.length;i<l/2;i++){
		   // alert("element numer : "+ i + " :marr: " + _mArr[i]);	    
		    if(elmnt[i].value!=""&&!_mArr[i]){
		    //alert('sdasdad');
		     isFill = false;
		     for(j=i+1; j<l; j++){ 
		     		if(elmnt[j].value!=""&&!_mArr[j]){
			           	if ((elmnt[i].value).replace(/-/g,'') == (elmnt[j].value).replace(/-/g,'')){
			           	_id.vis(c+'errorDiv'+(j));
			           	_id.o(c+'errorDiv'+(j)).parentNode.style.backgroundColor="#cccccc";isDp=true;
			           	_mArr[j]=true;
			          }else{
			          	_id.hid(c+'errorDiv'+(j));
			           	_id.o(c+'errorDiv'+(j)).parentNode.style.backgroundColor=tbl.style.backgroundColor;
		   	              ...
		   	              //alert('else' + _id.o(c+'errorDiv'+(i+1)));
			        	 _id.hid(c+'errorDiv'+(i+1));
			        	// alert('else1');
			           	_id.o(c+'errorDiv'+(i+1)).parentNode.style.backgroundColor=tbl.style.backgroundColor;
			           //	alert('else2');
...
function crtTabAg(txtBx,count){
    ...
    _tD=_tD+"</tr>";
	   ...
	   for (var i=0;i<6;i++){
		  _tD=_tD+'<tr>';
		   for(var j=0;j<2;j++){
		   var vInp='<b>'+((counter<10)?"0"+counter:counter)+'.</b> Ticket: <input type="text" id="'+count+'ticketNbr'+counter+'" name="ticketNumber" size="15" onkeydown="return checkNum(this.id)" onkeyup="return checkNum(this.id)"/> Virn:  <input type="text" id="'+count+'virnCode'+counter+'" name="virnCode" size="15" onkeydown="return checkVirn(this.id)" onkeyup="return checkVirn(this.id)"/><span id="'+count+'errorDiv'+counter+'" style="visibility:hidden">&nbsp;<img src="'+projectName+'/LMSImages/images/cross.gif" width="12" height="12"/></span></td>';
 ...
 //alert(tab+"---"+type+"--"+gameNum);
   ...
   for(var k=0;k<2;k++){
    ...
    var row = tbl.insertRow(iteration);
	  ...
	  counter=(elmnt.length)/2+1;
	  for(var j=0;j<2;j++){
	   var cellLeft = row.insertCell(j);
 	  var newdiv = document.createElement('div');
	  var vInp='<td width="20%" nowrap><b>'+counter+'.</b> Ticket: <input type="text" id="'+gameNum+'ticketNbr'+counter+'" name="ticketNumber" size="15" onkeydown="return checkNum(this.id)" onkeyup="return checkNum(this.id)"/> Virn:  <input type="text" id="'+gameNum+'virnCode'+counter+'" name="virnCode" size="15" onkeydown="return checkVirn(this.id)" onkeyup="return checkVirn(this.id)"/><span id="'+gameNum+'errorDiv'+counter+'" style="visibility:hidden">&nbsp;<img src="'+projectName+'/LMSImages/images/cross.gif" width="12" height="12"/></td>';
...
if(gmNoLnth<1)
_id.i('top_form','<h3>'+document.getElementsByTagName('h3')[0].innerHTML+'</h3><br/><center>No Game Available for Winning Acceptance</center>');
}
else{
gmNoLnth = numOfGame;
}
...
parent.frames[0].Version['$Source: /rep/LMS_Mgmt/WebRoot/com/skilrock/lms/web/scratchService/pwtMgmt/agent/js/Attic/agt_pwtMgmt_pwtCommonFnAgtTNV.js,v $'] = '$Id: agt_pwtMgmt_pwtCommonFnAgtTNV.js,v 1.1.2.3 2011/01/20 16:46:07 umesh Exp $';