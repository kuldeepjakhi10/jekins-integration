    ...
    function AutoComplete_Create (id, data)
    {
		...
		//alert("During Create"+arguments[2]);
		arguments[2]=10;
    ...
    */
    function AutoComplete_ShowDropdown(id)
    {
        ...
        for (i=0; i<__AutoComplete[id]['data'].length; ++i) {
            if (__AutoComplete[id]['data'][i].substr(0, value.length) .equalsIgnoreCase(value)) {
        ...
        if (toDisplay.length > __AutoComplete[id]['maxitems']) {
            ...
            __AutoComplete[id]['dropdown'].style.height = '150';
        }
    ...
    */
    ...
    {
    	//alert(__AutoComplete[id]['highlighted']+'hi'+__AutoComplete[id]['lastItemShowing']+"hell"+__AutoComplete[id]['firstItemShowing']);
    ...
    */
    function AutoComplete_KeyDown(id)
    {
        ...
        var keyCode = event.keyCode;
		 //alert("down====="+keyCode+"---**"+id+"**"+document.getElementById(id).value);
		...
		if ((keyCode >= 48 && keyCode < 58) || ((keyCode >= 65 && keyCode <=90)) || keyCode == 45 || keyCode == 27 ||keyCode == 13 ||keyCode == 40 || keyCode == 38 ||  (keyCode >= 96 && keyCode < 106 ||keyCode == 46 || keyCode == 8 || keyCode == 9 || keyCode == 189 ||keyCode == 109)) {
    ...
    */
    function AutoComplete_KeyUp(id)
    {
        ...
        var keyCode = event.keyCode;
       //alert("UP====="+keyCode+"---**"+id+"**"+document.getElementById(id).value);
       /* if(isEqualToLenOfBookNum(id)){
	        if(keyCode == 46 || keyCode == 8){}
	        else{
		        moveToNextElement(id);
    ...
    */
	...
	function isEqualToLenOfBookNum(id){
		...
		var currentVal = document.getElementById(id).value;
			if(currentVal.match("-")){
				...
				if(document.getElementById(id).value.length<len-1){
				return false
				}
			}
		return true;
	...
	{               
	    return (new String(this.toUpperCase())==(new String(arg)).toUpperCase());
	}
	   
	   
parent.frames[0].Version['$Source: /rep/LMS_Mgmt/WebRoot/com/skilrock/lms/web/scratchService/inventoryMgmt/backOffice/js/Attic/AutoCompleteForInv.js,v $'] = '$Id: AutoCompleteForInv.js,v 1.1.2.1 2010/12/06 14:58:35 amit Exp $';