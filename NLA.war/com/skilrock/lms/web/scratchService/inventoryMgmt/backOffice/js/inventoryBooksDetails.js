...
function onTypeChangeValue(typeValue) {	
	...
	if(typeValue=='BO') {
		...
		document.getElementById('agt1').style.display="none";	
		...
		document.getElementById('game_id').focus();
	 	...
	 	document.getElementById('agt1').style.display="block";	
...
function onAgentChangeValue(agentValue) {
	...
	if(type=='AGENT') {
		if(value=='-1') {		
			document.getElementById('agent_error').innerHTML=i18nOb.prop('error.pls.slct.an')+" "+tierMap['AGENT'].toUpperCase()+" "+i18nOb.prop('error.cmpny');		
			return false;
		}else return true;
...
function executeAjax(newUrl, div) {		
	...
	if(_resp.result){
	    	...
	    	//alert(resStr); 
			if(resStr=='NO_RET') {
				alert("NO "+tierMap['RETAILER'].toUpperCase()+" "+i18nOb.prop('error.org.exist'));
				document.getElementById('ret').style.display="none";
			}			
			else {
...
function getInventoryDetails(url, div) {
	...
	if((type=='AGENT')&& (agent=='-1')) {				
			...
			document.getElementById('ret_error').innerHTML=i18nOb.prop('error.pls.slct.an')+" "+tierMap['RETAILER'].toUpperCase()+" "+i18nOb.prop('error.cmpny');		
			return false;	
	}
	else {
		//alert("INSIDE else");	
		document.getElementById("loadingDiv").innerHTML='<img src="'+path+'/LMSImages/images/loadingAjax.gif"/>';
		var newUrl=url+"?type="+type+"& agtId="+escape(agent)+"& gameid="+gameid+"& retId="+retailer;
   _resp  = _ajaxCall(newUrl);
	if(_resp.result){
   	    	var resStr= _resp.data; 		
			if(resStr=='NONE') {
...
parent.frames[0].Version['$Source: /rep/LMS_Mgmt/WebRoot/com/skilrock/lms/web/scratchService/inventoryMgmt/backOffice/js/inventoryBooksDetails.js,v $'] = '$Id: inventoryBooksDetails.js,v 1.1.8.1.8.1.2.1 2014/12/10 10:37:22 yogesh Exp $';