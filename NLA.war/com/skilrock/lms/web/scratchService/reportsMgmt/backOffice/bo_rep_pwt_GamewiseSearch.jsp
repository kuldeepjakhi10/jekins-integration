...
<p>	 
	...
	<s:if test="#session.pwtResult.size>0">
			<s:div id="excel" cssStyle="text-align: left;width: 100%;font-size: 9pt"><s:a href="bo_rep_pwt_ExpExcel.action">Export As Excel</s:a></s:div>
	</s:if>
    ...
    <s:if test="#session.startValueOrderSearch==((#session.APP_ORDER_LIST5.size/10)*10)">Next Last</s:if><s:else><s:a theme="ajax" targets="down" href="bo_rep_pwt_Navigate.action?end=Next" >Next</s:a><s:a theme="ajax" targets="down" href="bo_rep_pwt_Navigate.action?end=last">Last</s:a></s:else>      
...
$RCSfile: bo_rep_pwt_GamewiseSearch.jsp,v $
$Revision: 1.2.8.5 $
</code>